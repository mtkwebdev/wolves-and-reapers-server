...
import {
	...
	createNewPlayer,
	defaultRes,
	successfulRes} from './dataTemplates.js'
...
export const setUpNewGame = async (id, code, username) => {
	...
	try {
		...
		if (isGameAlreadyExisting) {
			...
			const newPlayer = createNewPlayer(id, username, true)
...
export const addPlayerToGame = async (id, code, username) => {
	...
	try {
		const existingGame = await redisGet(code)[0];
		...
		if (!existingGame) {
			...
			if (listOfPlayers.includes(username)) {
				...
				const newPlayer = createNewPlayer(id, username)
				...
				let res = successfulRes(updatedGame, "player joined successfully")
				return res
				}
			}
		}
...
export const incrementTurns = async (code) => {
	...
	try {
		const game = await redisGet(code)[0];
		game.playerTurns += 1;
...
export const accumulateVotes = async (code, votedUsername) => {
	...
	try {
		...
		const updatedGame = await redisGet(code);
		if (voting && updatedGame ) {
			res = successfulRes(updatedGame)
			return res
		}
	} catch (error) {
		res.error = error;
