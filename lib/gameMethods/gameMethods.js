...
export const checkIsEliminationRound = (res) => {
	const game = res.game;

	const activePlayersCount = game.players.filter(
		(player) => player.isEliminated === false
	)?.length;

	const isEliminationRound = game.votes.length === activePlayersCount;
	return isEliminationRound;
};
